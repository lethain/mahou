    ...
    while (XMLNode = _plist_traverseNextNode(XMLNode, plistNode, containers))
    {
        ...
        if (!plistObject)
            ...
            if (currentContainer.slice)
                ...
                { if ((currentContainer).buckets[key] == NULL) { (currentContainer).keys.push(key); ++(currentContainer).count; } if (((currentContainer).buckets[key] = object) == NULL) --(currentContainer).count;};
    }
    return plistObject;
}
...
var _CPDictionaryAppendXMLData = function(XMLData, aDictionary)
{
    ...
    for (; i < count; ++i)
    {
        ...
        _CPPropertyListAppendXMLData(XMLData, ((aDictionary).buckets[keys[i]]));
    }
    XMLData.string += "</dict>";
}
...
_CPPropertyList280NorthSerializers["dictionary"] = function(aDictionary, serializers)
{
    ...
    for (; index < count; ++index)
    {
        ...
        string += _CPPropertyListSerializeObject(((aDictionary).buckets[key]), serializers);
    }
    return string + END_MARKER + ';';
}
...
objj_search.prototype.didReceiveBundleResponse = function(aResponse)
{
    ...
    var executablePath = ((bundle.info).buckets["CPBundleExecutable"]);
    if (executablePath)
    {
        ...
        var directory = (aResponse.filePath).substr(0, (aResponse.filePath).lastIndexOf('/') + 1),
            replacedFiles = ((bundle.info).buckets["CPBundleReplacedFiles"]),
            index = 0,
...
var objj_preprocess_implementation= function(tokens)
{
    ...
    if((token = tokens.skip_whitespace()) == TOKEN_OPEN_PARENTHESIS)
    {
        ...
        var superclass_name = ((SUPER_CLASSES).buckets[class_name]);
        if (!superclass_name)
    ...
    else
    {
        if(token == TOKEN_COLON)
        {
            ...
            CURRENT_SUPER_CLASS = "objj_getClass(\"" + superclass_name + "\")";
            { if ((SUPER_CLASSES).buckets[class_name] == NULL) { (SUPER_CLASSES).keys.push(class_name); ++(SUPER_CLASSES).count; } if (((SUPER_CLASSES).buckets[class_name] = superclass_name) == NULL) --(SUPER_CLASSES).count;};
            token = tokens.skip_whitespace();
        }
...
{
    var fragment = new objj_fragment();
    fragment.type = (FRAGMENT_FILE | (FRAGMENT_LOCAL * isLocal) | (FRAGMENT_IMPORT * isImport));
    fragment.info = aPath;
    fragment.bundle = aBundle;
    fragment.file = aFile;
    return fragment;
